      ...
      {data ? (
        <>
          ...
          <p className="mb-1 text-gray-700 font-medium">
            Perusahaan: {data.perushaaan}
          </p>
          <p className="mb-1 text-gray-700">Lokasi: {data.lokasi}</p>
          <p className="mb-1 text-gray-700">Email: {data.email}</p>
          <p className="mb-2 text-gray-600 whitespace-pre-wrap">{data.deskripsi}</p>
        ...
        <p className="text-gray-400">Tidak ada data lowongan.</p>
      )}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-xl w-full max-w-2xl shadow-2xl border-2 border-yellow-400">
            <h3 className="text-2xl font-bold text-yellow-700 mb-4">
              {data ? "Edit Lowongan" : "Tambah Lowongan"}
            </h3>

            <form onSubmit={handleSubmit} className="space-y-4">
              {["judul", "perushaaan", "lokasi", "email"].map((field) => (
                <input
                  key={field}
                  type="text"
                  name={field}
